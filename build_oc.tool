...
package() {
  ...
  local ver
  ver=$(grep OPEN_CORE_VERSION ./Include/devicemanager/Library/OcMainLib.h | sed 's/.*"\(.*\)".*/\1/' | grep -E '^[0-9.]+$')
  if [ "$ver" = "" ]; then
...
src=$(curl -LfsS https://raw.githubusercontent.com/devicemanager/ocbuild/master/efibuild.sh) && eval "$src" || exit 1
